...
static void NRF_tx_up();
static void NRF_rx_up();
static uint8_t NRF_read_reg( uint8_t reg );
static void NRF_read_reg_to_buf( uint8_t reg, uint8_t * buf, uint8_t len );
static void NRF_write_reg( uint8_t reg, uint8_t data );
static void NRF_write_reg_from_buf( uint8_t reg, uint8_t * buf, uint8_t len );
static void NRF_tx_flush();
static void NRF_rx_flush();
...
void NRF_rx_event( char * buf ) {
	...
	if (NRF_read_reg( REG_STATUS ) & (1 << RX_DR)) {
		...
		do {
			...
			if( len && NRF_rx_event_callback ) (*NRF_rx_event_callback)( buf, len );
